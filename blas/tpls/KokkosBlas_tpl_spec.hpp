...
/// \brief This function converts KK uplo mode to oneMKL uplo mode
inline oneapi::mkl::uplo uplo_kk_to_onemkl(char uplo_kk) {
  switch (toupper(uplo_kk)) {
    case 'U': return oneapi::mkl::uplo::upper;
    case 'L': return oneapi::mkl::uplo::lower;
    default:;
  }
  throw std::invalid_argument("Invalid uplo for oneMKL (should be one of U, L)");
}

/// \brief This function converts KK side mode to oneMKL side mode
inline oneapi::mkl::side side_mode_kk_to_onemkl(const char kkSide[]) {
  if ((kkSide[0] == 'L') || (kkSide[0] == 'l')) return oneapi::mkl::side::left;
  return oneapi::mkl::side::right;
}

/// \brief This function converts KK diag mode to oneMKL diag mode
inline oneapi::mkl::diag diag_kk_to_onemkl(char kkDiag) {
  if ((kkDiag == 'U') || (kkDiag == 'u')) return oneapi::mkl::diag::unit;
